...
def parse ():
    ...
    for l in lines:
        if l == "\n":
            ...
            current = []
        else: current.append(int(l))

    if current: elves.append(current)

    return elves

def solve ():
    print("--- Day 1: Calorie Counting ---")
    elves = sorted(sum(e) for e in parse())
    calories, top_three = elves[-1], sum(elves[-3:])
    print(f'part 1: {calories}, part 2: {top_three}')
